...
\begin{abstract}
We develop the quotient geometry of statistical manifolds under collapse maps---smooth maps $\pi: \M \to \R^k$ with $k < \dim(\M)$. The main results are: (1) a \emph{fiber structure theorem} showing that the vertical distribution $\mathcal{V} = \ker(d\pi)$ foliates $\M$ into submanifolds along which the Fisher metric becomes degenerate, with points on the same fiber being statistically non-identifiable in the sense of Watanabe; (2) a \emph{quotient metric theorem} establishing when the Fisher metric descends to a well-defined Riemannian metric on the quotient $\M/\sim_\pi$, and characterizing obstructions in terms of $\alpha$-connection geometry; and (3) \emph{covering number bounds} relating the metric geometry of the quotient to the number of distinguishable equivalence classes at finite resolution. We show that a recent result on minimal embedding dimension for recurrent processes is a special case of this framework. The theory provides a unified geometric treatment of dimensional reduction, sufficient statistics, and singularities in statistical models.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Quotient Metric Theorem} (Theorem~\ref{thm:quotient-metric}): Under regularity conditions, the Fisher metric descends to a well-defined Riemannian metric on the quotient space $\M/\sim_\pi$. We characterize when this descent is compatible with the $\alpha$-connection structure and identify obstructions arising from non-trivial holonomy.

    \item \textbf{Covering Number Bounds} (Theorem~\ref{thm:covering}): The number of $\varepsilon$-distinguishable classes scales as $N(\varepsilon) \sim C \cdot \text{Vol}(\M) / \varepsilon^r$, where $r$ is the projection rank. This quantifies the ``effective alphabet size'' induced by dimensional collapse.
\end{enumerate}
...
$\tilde{p}_y$ & Collapsed family: $p_\theta = \tilde{p}_{\pi(\theta)}$ \\
$\tilde{g}$ & Fisher metric of the collapsed model on $\pi(\M)$ \\
$g_{\text{obs}} = \pi^* \tilde{g}$ & Pullback (observed) metric on $\M$; has rank $r \leq k$ \\
$\mathcal{V} = \ker(d\pi)$ & Vertical distribution (tangent to fibers) \\
...
\subsection{Statistical Collapse via Observation}

Dimensional collapse in statistical models arises from observation: when we observe a system through a lower-dimensional statistic, parameters that produce identical observed distributions become indistinguishable.
...
A \emph{collapse map} is a smooth surjective submersion $\pi: \M \to \R^k$ with $k < \dim(\M)$ such that the statistical family factors through $\pi$: there exists a family $\{\tilde{p}_y : y \in \pi(\M)\}$ with $p_\theta = \tilde{p}_{\pi(\theta)}$ for all $\theta \in \M$.

Equivalently, $\pi(\theta_1) = \pi(\theta_2)$ implies $p_{\theta_1} = p_{\theta_2}$. The map $\pi$ serves as a coordinate representation of the quotient structure.
\end{definition}
...
\begin{theorem}[Fiber Structure and Non-Identifiability]\label{thm:fiber}
Let $\pi: \M \to \R^k$ be a collapse map (Definition~\ref{def:collapse}) with constant rank $r \leq k$ on a neighborhood $U \subset \M$. Let $g$ denote the Fisher metric of the full model on $\M$, and let $\tilde{g}$ denote the Fisher metric of the collapsed model $\{\tilde{p}_y\}$ on $\pi(U) \subset \R^k$. Then:
\begin{enumerate}
    ...
    \item[(ii)] \textbf{Non-identifiability:} For $\theta_1, \theta_2$ on the same fiber, the distributions coincide: $p_{\theta_1} = p_{\theta_2}$. Equivalently, for any statistic measurable with respect to the $\sigma$-algebra generated by $\pi$, the induced distributions are identical. No estimator can distinguish parameters on the same fiber.
    ...
    \item[(iv)] \textbf{Singularity:} The observed model $\{\tilde{p}_y\}$ viewed in the original $\theta$-coordinates is \emph{singular} in the sense of Watanabe \citep{watanabe2009algebraic}: the effective Fisher information has rank $r < n$, and classical asymptotics (Cram\'er-Rao bounds, BIC) fail along vertical directions.
\end{enumerate}
\end{theorem}
...
(ii) By Definition~\ref{def:collapse}, $\pi(\theta_1) = \pi(\theta_2)$ implies $p_{\theta_1} = \tilde{p}_{\pi(\theta_1)} = \tilde{p}_{\pi(\theta_2)} = p_{\theta_2}$.
...
(iv) Vectors in $\mathcal{V}$ represent parameter changes that leave $\pi(\theta)$---and hence $p_\theta$---unchanged, so they contribute zero to the Fisher information of the observed model.
\end{proof}
...
Let $\pi: \M \to \R^k$ be a submersion (constant rank $k$). The Fisher metric $g$ descends to a Riemannian metric $\bar{g}$ on $\M/\sim_\pi$ if and only if the metric is bundle-like with respect to the fiber foliation. When this holds, the quotient metric is defined by $\bar{g}(d\pi(X), d\pi(Y)) = g(X, Y)$ for $X, Y \in \mathcal{H}$.
\end{theorem}

\begin{proof}
The metric $\bar{g}$ is well-defined if and only if $g(X, Y)$ depends only on the equivalence classes, not on the choice of fiber representatives. For horizontal lifts $X, Y$ of vectors in the quotient, this requires $\mathcal{L}_V g(X, Y) = 0$ for all vertical vector fields $V$---i.e., $g(X, Y)$ is constant along fibers. This is precisely the bundle-like condition.
\end{proof}
...
As resolution improves ($\varepsilon \to 0$), the number of distinguishable classes grows as $N(\varepsilon) \sim \varepsilon^{-r}$. Lower-rank projections (more severe collapse) yield fewer classes at any resolution.
\end{corollary}
...
\begin{corollary}[Minimal Embedding as Covering Collapse]\label{cor:embedding}
Let $\gamma: S^1 \to \M$ be a cyclic process with monotone meta-time (a coordinate that increases strictly along the trajectory and resets at cycle completion). Consider the projection $\pi: \M \to \R^k$.
\begin{enumerate}
    \item[(i)] \textbf{$k = 2$:} Any such trajectory must self-intersect: the curve must return to its starting spatial position while meta-time has advanced. At each self-intersection, distinct temporal states (different meta-time values) map to the same projected point. This forces the \emph{effective} image rank to $r = 0$ at intersection loci. By Theorem~\ref{thm:covering}, the number of $\varepsilon$-distinguishable classes is bounded: $N(\varepsilon) = O(1)$ as $\varepsilon \to 0$.

    \item[(ii)] \textbf{$k = 3$:} The Whitney embedding theorem guarantees that generic embeddings of $S^1$ into $\R^3$ are injective (no self-intersections). The projection has full rank $r = 1$ along the curve, and Theorem~\ref{thm:covering} yields $N(\varepsilon) \sim \varepsilon^{-1} \to \infty$.
\end{enumerate}
\end{corollary}

The $k = 3$ threshold marks the transition from ``too few classes'' (forced discretization) to ``enough classes'' (preserved continuity). In the language of the present framework: the meta-time coordinate becomes part of the vertical distribution $\mathcal{V}$ when $k = 2$, because self-intersections collapse temporally distinct states to the same fiber. At $k = 3$, meta-time remains in the horizontal distribution $\mathcal{H}$ and contributes to distinguishability.
...
\begin{enumerate}
    ...
    \item \textbf{Covering Number Bounds:} The number of distinguishable classes at resolution $\varepsilon$ scales as $\varepsilon^{-r}$ where $r$ is the projection rank.
\end{enumerate}

The framework unifies dimensional reduction, sufficient statistics, and singular models under a common geometric structure. The minimal embedding theorem for recurrent processes emerges as a special case: the $k = 3$ threshold marks where covering numbers transition from bounded to unbounded.
